...
同日追加：硬體 Gate 的完整規則及 PDF「資料來源與判斷方式」文案，見
[APP_PSD_IMPLEMENTATION.md](APP_PSD_IMPLEMENTATION.md) §11。
依 App 組確認，硬體規則說明的主要交付位置為 PDF 匯出模板，App 內說明頁可另行
安排；現有 PDF 模板的對應修改與程式來源見 §11.5。本次沒有修改 HTML／Swift 或產出 PDF。
控制觸發的設計優先序更新為「優先減少需要介入時的漏觸發，容許部分額外觸發」，
評估方法見同文件 §11.4；這是目標補充，未變更現行門檻或宣稱患者效能已驗證。
使用告知與研究限制草案、App 呈現方式及尚待補齊的個資告知見同文件 §13；
草案不代表法律審核、人體使用或隱私措施已完成。
...
>
> 硬體規則請放進 PDF 匯出模板：圖表旁放解讀註記，報告末附分析與控制規則，
> §11.2 有文案與 Gate 表格，版本要對應當時資料。App 內說明頁可另行安排。
> motor_enabled 若列入報告，仍只能畫命令作用區段；
> 若需要原始 Gate 起迄，要由硬體提供獨立狀態，不能從馬達旗標反推。
> 報告說明與事件偵測是不同工作，這次並未完成患者事件判定功能。

> 目前模板／SVG 將 0.20 當警戒線，並用 0.20／0.50 分中度／劇烈震顫，尚無
> 已確認的臨床分級依據，請一併移除。事件表暫改成分析紀錄／紀錄時間，空清單
> 不表示無震顫；模板文字及產生內容的 ExportSettingsViewModel 都要同步修改。
> 用藥紀錄也要區分使用者填寫的實際時間與排程打卡時間，避免匯出後混用。
...
> App 請採「使用告知與研究限制」頁名，§13 有文字草案。先讓使用者知道研究
> 原型、誤觸發／漏觸發、用藥解讀及拉線風險；不能只寫「一切責任自負」。
> 閱讀確認不啟動控制，也不取代研究同意或個資告知。實際停止／解除方式、聯絡
> 窗口及資料保存安排仍需補齊；不要把尚未確認的功能或措施寫成保證。
...
| 5 | App PDF 模板與硬體交付 | 依 APP_PSD_IMPLEMENTATION §11.2／§11.5 補圖例與附錄，修正未驗證警戒／分級及事件命名；核對資料當時版本與用藥時間來源 | 實際 PDF 可讀、版本可追溯、缺值不補零；硬體包絡與 App RMS 分開，不以命令旗標冒充 Gate 或患者事件 |
...
請提供 build／commit、取樣率、選用軸及配置識別與參數。§11 的 PDF 附錄可以先依
參考版本製作，不能在版本未知時宣稱其門檻就是報告資料當時的實際配置。
