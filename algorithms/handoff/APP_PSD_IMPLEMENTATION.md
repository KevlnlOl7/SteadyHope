...
§11 補上硬體 Gate 規則、匯出 PDF 模板的說明文案及版本核對要求。
...
### 11.2 匯出 PDF 的「資料來源與判斷方式」說明

依 App 組確認的需求，**本節主要交付位置是匯出 PDF 模板**。App 內同內容的
說明頁可另行安排，不列為本次硬體規則說明的必要交付；§13 的事前使用告知另行處理。
以下是待實作規格，本次未修改 HTML／Swift 或產出 PDF。

PDF 圖表旁放簡短的資料解讀註記，報告末附「資料來源、分析與控制規則」附錄。
醫師拿到 PDF 或列印本時，應能直接知道指標單位、資料缺口及適用規則，不必返回
App 或登入私人 GitHub 才能理解。程式連結及文獻作補充來源，不能代替報告內文。

圖表旁可使用的簡短文案：

> 本報告呈現配戴部位動作訊號的頻率與角速度強度，供回診溝通參考。
> 活動及用藥紀錄來自使用者登錄；其時間來源與資料缺口需一併解讀。
>
> 頻率顯示「--」時，請查看圖例及資料狀態：可能是資料不足、中斷，或暫時沒有清楚的
> 主要頻率；不代表一定沒有抖動。
>
> 若報告包含裝置作動標記，其表示控制命令送出的時段，不能直接當作抖動的開始、
> 結束或改善程度。
> 本紀錄不能單獨判定病程或服藥療效。

附錄可列出設計目標：「本專題優先減少需要介入時未觸發的情況，因此容許
部分日常動作也觸發裝置。」這是設計取捨，不能寫成已證明不會漏掉患者震顫。

| PDF 說明項目 | 報告需交代的內容 |
|---|---|
| 頻率 | 目前規格在 3–7 Hz 搜尋主要頻率；資料與頻率品質條件通過後才顯示。未通過時保留原因，不顯示為 0 Hz |
| 強度 | 交付規格顯示 4–6 Hz 角速度 RMS，單位 deg/s；不是手部位移距離或疾病分期。附錄另列三項可信度公式及 0.20／0.30／0.45 工程門檻，見 §7；報告必須對應實際分析器版本 |
| 裝置作動 | 若新增此資料，圖例寫「裝置作動（命令紀錄）」並說明為控制鏈成功套用非零命令。現有 PDF 尚無此圖層，不能宣稱已有；也不能命名為「震顫事件」或「抑震成功」 |
| 活動與服藥 | 區分使用者填寫的實際服用時間與排程打卡可能沿用的預定時間。未確認時寫「用藥紀錄時間」並註明來源；未填寫不推測成已服藥、未服藥或特定活動 |
| 判斷規則版本 | 列資料期間、時區、報告產生時間及資料當時的 App／分析規則／韌體／Gate 配置版本。未知寫「未記錄／待確認」；不能用匯出當天的最新版本冒充歷史資料版本 |
| 資料涵蓋 | 如列有效量測時間，須按實際可觀測資料計算；資料缺口另列，不把所選日曆期間、保存筆數或重疊 4 秒窗口總和當作震顫時長 |

附錄要將「App 頻率可信度」與「硬體 Gate」分開列出，並標示硬體門檻是本專題
...
PDF 附錄中的硬體規則可使用下表。表頭須明示對應的紀錄版本；未確認時只能標為
「參考規則：Ryan c08bfcc，資料當時韌體／參數待確認」，不能寫成本次已套用規則。

| 硬體控制 Gate（已核對參考版本） | 判斷方式 |
|---|---|
| 訊號 | X 軸角速度、名目 100 Hz；比較 4–6 Hz 與 1–3 Hz 帶通後的衰減峰值包絡 |
| 比例 | `E_t / (E_t + E_v + 1e-9)`；`E_t` 是 4–6 Hz 包絡，`E_v` 是 1–3 Hz 包絡；包絡每筆為 `max(abs(濾波輸出), 0.94 × 前筆包絡)` |
| 啟動 | `E_t ≥ 6 deg/s` 且比例 `≥ 0.55`，連續 20 筆（名目 200 ms） |
| 解除 | `E_t < 3 deg/s` 或比例 `< 0.45`，連續 15 筆（名目 150 ms）；輸入或控制故障另行中止 |
| 解讀 | 這是控制觸發規則；包絡不是 App RMS，確認時間不是患者事件時長，Gate 也不等於實際命令、位移或抑震效果 |

附錄採可閱讀的正文尺寸並允許正常換頁，不能把整張規則表縮成細小頁尾。
這張表描述已核對的工程配置，**不是患者震顫診斷或疾病嚴重度標準**。
...
冒充硬體回報。患者震顫事件仍依 §12 另定義與驗證，不能靠新增 PDF 說明補成已完成。

待 PDF 驗收：圖例與附錄在實際匯出檔完整且可讀；缺資料與無可靠頻率能區分；
Gate 與命令狀態未互相代用；版本未知不顯示為已核對；無用藥紀錄不生成服藥標記；
分析紀錄不計成患者事件。長日期範圍及跨多個規則版本時須保留分段／對應關係。
...
### 11.5 現有 PDF 模板的對應修改位置

2026-09-12 再次核對 GitHub，App 分支仍為 `56805c93c46af81fb922af7780a8841a07258b39`。
模板是 `software/iOS-App/Glove/Resources/report_template.html`，內容及 SVG 由
`ViewModels/ExportSettingsViewModel.swift` 組成，再由 `PDFDataGenerator.swift`
匯出。這次僅修改交付 MD，下列 HTML／Swift 變更與實際 PDF 驗收尚待 App 組執行。

| 現有輸出 | 待修正的報告內容 |
|---|---|
| RMS 圖例及 SVG 的「0.20 deg/s 警戒門檻／警戒線」 | 移除這條警戒線及其圖例。依 §7 交付規格，0.20 是三軸整體 vector RMS 的頻率回報條件之一，不能畫在 band RMS 圖上作患者警戒門檻；現有 App 的公式差異另見 §7.1，附錄按實際分析規則說明 |
| ≥0.20 標「中度震顫」、≥0.50 標「劇烈震顫」 | 移除未驗證的疾病嚴重度分級及對應顏色語意；保留有單位的實測數值，品質狀態依真實資料顯示 |
| 「手部震顫事件紀錄／發作時間／狀態等級」 | 在事件規則尚未完成前改成「動作分析紀錄／紀錄時間」；無有效依據的狀態等級欄移除，若另列品質狀態須有資料來源 |
| 空清單顯示「未捕捉到顯著高震顫事件」 | 改為「此期間無可列出的分析紀錄」，不推論為無震顫；有資料缺口時另說明 |
| 用藥表把 `med.date` 全稱為「服藥時間」 | 依來源區分實際時間與排程登錄時間。現有排程打卡路徑會用預定時間建立 date，未確認時改列「用藥紀錄時間」；使用者可填寫實際時間的功能應保留 |
| 沒有分析及控制規則附錄 | 依 §11.2 加圖表註記、指標定義及硬體 Gate 附錄；完整規則須對應資料版本，不能只更新模板文字就宣稱舊分析資料符合新公式 |

固定版本程式來源：

- [PDF 圖表、警戒圖例與事件表](https://github.com/KevlnlOl7/SteadyHope/blob/56805c93c46af81fb922af7780a8841a07258b39/software/iOS-App/Glove/Resources/report_template.html#L316-L373)
- [SVG 警戒線](https://github.com/KevlnlOl7/SteadyHope/blob/56805c93c46af81fb922af7780a8841a07258b39/software/iOS-App/Glove/ViewModels/ExportSettingsViewModel.swift#L342-L346)
- [逐筆分析清單及未驗證分級](https://github.com/KevlnlOl7/SteadyHope/blob/56805c93c46af81fb922af7780a8841a07258b39/software/iOS-App/Glove/ViewModels/ExportSettingsViewModel.swift#L716-L736)
- [用藥列資料來源](https://github.com/KevlnlOl7/SteadyHope/blob/56805c93c46af81fb922af7780a8841a07258b39/software/iOS-App/Glove/ViewModels/ExportSettingsViewModel.swift#L661-L678)、[排程打卡時間來源](https://github.com/KevlnlOl7/SteadyHope/blob/56805c93c46af81fb922af7780a8841a07258b39/software/iOS-App/Glove/ViewModels/MedicationViewModel.swift#L126-L130)
- [模板載入與內容替換](https://github.com/KevlnlOl7/SteadyHope/blob/56805c93c46af81fb922af7780a8841a07258b39/software/iOS-App/Glove/ViewModels/ExportSettingsViewModel.swift#L884-L921)、[PDF 產生器](https://github.com/KevlnlOl7/SteadyHope/blob/56805c93c46af81fb922af7780a8841a07258b39/software/iOS-App/Glove/Views/Components/PDFDataGenerator.swift#L23-L85)

App 組需以空資料、有缺口、無可靠頻率、用藥實際／排程時間及多頁資料等案例實際
匯出 PDF，檢查數值、標籤與分頁。現有每小時代表頻率與 RMS 圖的彙整方式也須
按真正輸出說明，不把一小時代表值稱為該小時全程頻率。報告內的限制摘要用於解讀
資料，§13 的事前使用告知、個資告知及適用研究同意仍是另項工作。
