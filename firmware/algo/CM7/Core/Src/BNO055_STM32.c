...
/**
  ...
  * @retval None
  */
HAL_StatusTypeDef ResetBNO055(void)
{
    uint8_t reset = 0x20;
    uint8_t chip_id = 0;
    HAL_StatusTypeDef status;
    ...
    /* 先確認裝置位址有回應，避免無限等待。 */
    status = HAL_I2C_IsDeviceReady(&bno_i2c, P_BNO055, 3, 100);
    if (status != HAL_OK)
    {
        return status;
    }

    status = HAL_I2C_Mem_Write(&bno_i2c, P_BNO055, SYS_TRIGGER_ADDR,
    ...
    /* 最多等待 1500 ms，不能永久卡在 reset。 */
    while ((HAL_GetTick() - start_tick) < 1500U)
    {
        status = HAL_I2C_Mem_Read(&bno_i2c, P_BNO055, CHIP_ID_ADDR,
 ...
 */
HAL_StatusTypeDef ReadData(BNO055_Sensors_t *sensorData, BNO055_Sensor_Type sensors)
{
    uint8_t buffer[8] = {0};
    HAL_StatusTypeDef status;
    ...
    {
        status = HAL_I2C_Mem_Read(&bno_i2c, P_BNO055, BNO_GRAVITY,
                                  I2C_MEMADD_SIZE_8BIT, buffer, 6, 20);
        if (status != HAL_OK) return status;
        sensorData->Gravity.X = (float)((int16_t)((buffer[1] << 8) | buffer[0])) / 100.0f;
        sensorData->Gravity.Y = (float)((int16_t)((buffer[3] << 8) | buffer[2])) / 100.0f;
        sensorData->Gravity.Z = (float)((int16_t)((buffer[5] << 8) | buffer[4])) / 100.0f;
    }

    if (sensors & SENSOR_QUATERNION)
    {
        status = HAL_I2C_Mem_Read(&bno_i2c, P_BNO055, BNO_QUATERNION,
                                  I2C_MEMADD_SIZE_8BIT, buffer, 8, 20);
        if (status != HAL_OK) return status;
        sensorData->Quaternion.W = (float)((int16_t)((buffer[1] << 8) | buffer[0])) / 16384.0f;
        sensorData->Quaternion.X = (float)((int16_t)((buffer[3] << 8) | buffer[2])) / 16384.0f;
        sensorData->Quaternion.Y = (float)((int16_t)((buffer[5] << 8) | buffer[4])) / 16384.0f;
        sensorData->Quaternion.Z = (float)((int16_t)((buffer[7] << 8) | buffer[6])) / 16384.0f;
    }

    if (sensors & SENSOR_LINACC)
    {
        status = HAL_I2C_Mem_Read(&bno_i2c, P_BNO055, BNO_LINACC,
                                  I2C_MEMADD_SIZE_8BIT, buffer, 6, 20);
        if (status != HAL_OK) return status;
        sensorData->LineerAcc.X = (float)((int16_t)((buffer[1] << 8) | buffer[0])) / 100.0f;
        sensorData->LineerAcc.Y = (float)((int16_t)((buffer[3] << 8) | buffer[2])) / 100.0f;
        sensorData->LineerAcc.Z = (float)((int16_t)((buffer[5] << 8) | buffer[4])) / 100.0f;
    }

    if (sensors & SENSOR_GYRO)
    {
        status = HAL_I2C_Mem_Read(&bno_i2c, P_BNO055, BNO_GYRO,
                                  I2C_MEMADD_SIZE_8BIT, buffer, 6, 20);
        if (status != HAL_OK) return status;
        sensorData->Gyro.X = (float)((int16_t)((buffer[1] << 8) | buffer[0])) / 16.0f;
        sensorData->Gyro.Y = (float)((int16_t)((buffer[3] << 8) | buffer[2])) / 16.0f;
        sensorData->Gyro.Z = (float)((int16_t)((buffer[5] << 8) | buffer[4])) / 16.0f;
    }

    if (sensors & SENSOR_ACCEL)
    {
        status = HAL_I2C_Mem_Read(&bno_i2c, P_BNO055, BNO_ACCEL,
                                  I2C_MEMADD_SIZE_8BIT, buffer, 6, 20);
        if (status != HAL_OK) return status;
        sensorData->Accel.X = (float)((int16_t)((buffer[1] << 8) | buffer[0])) / 100.0f;
        sensorData->Accel.Y = (float)((int16_t)((buffer[3] << 8) | buffer[2])) / 100.0f;
        sensorData->Accel.Z = (float)((int16_t)((buffer[5] << 8) | buffer[4])) / 100.0f;
    }

    if (sensors & SENSOR_MAG)
    {
        status = HAL_I2C_Mem_Read(&bno_i2c, P_BNO055, BNO_MAG,
                                  I2C_MEMADD_SIZE_8BIT, buffer, 6, 20);
        if (status != HAL_OK) return status;
        sensorData->Magneto.X = (float)((int16_t)((buffer[1] << 8) | buffer[0])) / 16.0f;
        sensorData->Magneto.Y = (float)((int16_t)((buffer[3] << 8) | buffer[2])) / 16.0f;
        sensorData->Magneto.Z = (float)((int16_t)((buffer[5] << 8) | buffer[4])) / 16.0f;
    }

    if (sensors & SENSOR_EULER)
    {
        status = HAL_I2C_Mem_Read(&bno_i2c, P_BNO055, BNO_EULER,
                                  I2C_MEMADD_SIZE_8BIT, buffer, 6, 20);
        if (status != HAL_OK) return status;
        sensorData->Euler.X = (float)((int16_t)((buffer[1] << 8) | buffer[0])) / 16.0f;
        sensorData->Euler.Y = (float)((int16_t)((buffer[3] << 8) | buffer[2])) / 16.0f;
        sensorData->Euler.Z = (float)((int16_t)((buffer[5] << 8) | buffer[4])) / 16.0f;
    }
...
void getCalibration(Calib_status_t *calib) {
    ...
    HAL_StatusTypeDef status = HAL_I2C_Mem_Read(&hi2c4, P_BNO055, CALIB_STAT_ADDR, 1, &calData, 1, HAL_MAX_DELAY);
